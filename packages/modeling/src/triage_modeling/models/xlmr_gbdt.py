...
def _resolve_gbdt_backend(class_weight: str | None) -> GbdtBackend:
    ...
    try:
        ...
        raise RuntimeError("xlmr_gbdt icin lightgbm veya xgboost bagimliligi bulunamadi.") from exc


def build_xlmr_gbdt_pipeline(
    ...
    preprocessor = build_bert_tabular_preprocessor(
        bert_model_name=transformer_model_name,
        device=device,
    )
